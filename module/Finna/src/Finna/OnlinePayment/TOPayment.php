...
use Finna\OnlinePayment\TurkuOnline\TurkuOnline;
 ...
 * @link     http://vufind.org/wiki/vufind2:developer_manual Wiki
 */
class TOPayment extends Paytrail
{
 /**
  * Create a function which tries to fetch the correct url for Paytrail
  * HTTP Authorization -header kenttään tulee SHA-256 (asiointisovelluksen nimi + aikaleima (muodossa YYYY-MM-DDThh:mm:ssZ) + sanoma + apikey), 
  * esim. SHA-256("testiasiointisovellus2017-04-06T10:51:00Z{…json…}apikey123"). 
  * Aikaleiman tulee olla sama, mitä on käytetty X-TURKU-TS -headerissa (ks. HTTP Otsikkokentät -osio).
  */
    ...
    public function startPayment(
      ...
      if (!isset($this->config->productCode)
          ...
          foreach ($fines as $fine) {
              ...
              $module->addProduct(
                  $fineDesc, $code, 1, $fine['balance'], 0, TurkuOnline::TYPE_NORMAL
              );
          }
          if ($transactionFee) {
              ...
              $module->addProduct(
                  ...
                  $transactionFee, 0, TurkuOnline::TYPE_HANDLING
              );
          }
      }
