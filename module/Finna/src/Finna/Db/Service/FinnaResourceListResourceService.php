    ...
    DbServiceAwareInterface
{
    ...
    public function createOrUpdateLink(
        ...
        $resource = $this->getDoctrineReference(ResourceEntityInterface::class, $resource);
        $user = $this->getDoctrineReference(UserEntityInterface::class, $user);
        $list = $this->getDoctrineReference(FinnaResourceListEntityInterface::class, $list);
        $params = compact('resource', 'list', 'user');
        $result = current($this->entityManager->getRepository(FinnaResourceListResourceEntityInterface::class)
            ->findBy($params));
