...
function addGroup(_firstTerm, _firstField, _join) {
  ...
  $newGroup.find('.radio-wrapper input').each( function updateRadioButton(){
    $( this ).attr('name', 'bool' + nextGroup + '[]');
    $( this ).attr('id', $( this ).attr('id') + nextGroup );
  });
  $newGroup.find('.radio-wrapper label').each( function updateLabel(){
    $( this ).attr('for', $( this ).attr('for') + nextGroup );
  });
